...
import { convertFontWeight, convertLineHeight, convertFontSize, convertLetterSpacing } from './text';
...
export const convertCss = (
    ...
    errors: string[],
    settings: { autoConvertColor: boolean; autoConvertSpacing: boolean; remConversion: number; }
) => {
...
const processValue = (property: string, value: string, tailWindStyles: string[], errors: string[], settings: { autoConvertColor: boolean; autoConvertSpacing: boolean; remConversion: number; }) => {
    if (
    ...
    switch (property) {
        ...
        case 'letter-spacing':
            return convertLetterSpacing(value, settings)
        default:
            return value;
    }
};
